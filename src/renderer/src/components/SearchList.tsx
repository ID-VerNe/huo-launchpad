  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar space-y-1">
      {results.map((item, index) => (
        ...
        >
          <img src={item.icon} alt={item.name} className="w-8 h-8 object-contain flex-shrink-0" />
          ...
          <button 
            ...
            className={cn(
              "p-1.5 rounded transition-all opacity-0 group-hover:opacity-100 flex-shrink-0", 
